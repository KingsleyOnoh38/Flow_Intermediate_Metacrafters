...
transaction(senderAccount: Address, amount: UFix64) {
    ...
    // Define references
    let senderVault: &WofiToken.Vault{WofiToken.CollectionPublic}
    ...
    let adminResource: &WofiToken.Admin
    let flowMinter: &FlowToken.Minter

    prepare(acct: AuthAccount) {
        // Borrow references and handle errors
        self.adminResource = acct.borrow<&WofiToken.Admin>(from: /storage/AdminStorage)
        ...
        self.senderVault = getAccount(senderAccount)
            ...
            .borrow<&WofiToken.Vault{WofiToken.CollectionPublic}>()
